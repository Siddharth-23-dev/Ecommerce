    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        .then(categories => {
            ...
            if (categories.length === 0) {
                grid.innerHTML = '<div style="grid-column: 1/-1; text-align: center; padding: 2rem; color: #666;">No categories found.</div>';
                return;
            }

            // Using the shop route as the base link
            const shopUrl = "{{ route('shop') }}";
            const assetBase = "{{ asset('uploads/categories/') }}";
            const defaultMImage = "{{ asset('assets/website/images/home/demo3/category_1.png') }}";

            categories.forEach(category => {
                const imgUrl = category.image ? `${assetBase}/${category.image}` : defaultMImage;
                
                const card = `
                    <a href="${shopUrl}?category=${category.slug}" class="category-card">
        ...
        .catch(error => {
            ...
            const grid = document.getElementById('dynamic-category-grid');
            grid.innerHTML = '<div style="grid-column: 1/-1; text-align: center; padding: 2rem; color: red;">Failed to load categories. Please try again later.</div>';
        });
    });
  </script>
  @endpush
